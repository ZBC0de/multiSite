        ...
        io.on('connection', (socket) => {//start of socket.io
            ...
            socket.on('js', function (data) {
                ...
                switch (msg.type) {
                    ...
                    case 'submitChat':
                        let message = `${msg.user} - ${msg.data}<br>`;
                    ...
                    case 'addDB':
                        if (msg.user.length === 0 || msg.pass.length === 0) {
                            ...
                            con.query('INSERT INTO main.userinfo SET?', {
                                username: msg.user,
                                password: msg.pass
                            }, function (err, result) {
                                if (err) {
                    ...
                    case 'checkDB':
                        con.query(`SELECT * FROM main.userinfo WHERE username = '${msg.user}' AND password = '${msg.pass}'`, function (err, result) {
                            if (err) {
                                console.log(err);
                    ...
                    case 'changeData':
                        switch (msg.change) {
                            case 'pfp':
                                con.query(`UPDATE main.userinfo SET profilePicture = '${msg.data}' WHERE username = '${msg.user}'`);
                                con.query(`SELECT * FROM userinfo WHERE username = '${msg.user}'`, function (err, result) {
                                    if (err) {
                                        console.log(err);
                                    } else {
                                ...
                                break;
                            case 'pass':
                                con.query(`UPDATE main.userinfo SET password = '${msg.data}' WHERE username = '${msg.user}'`, function (err, result) {
                                    if (err) {
                                        console.log(err);
                                    } else {
                                ...
                                break;
                            case 'location':
                                let location = (`${msg.city} ${msg.state}, ${msg.country}`);
                                console.log(location);
                                con.query(`UPDATE main.userinfo SET location = '${location}' WHERE username = '${msg.user}'`, function (err, result) {
                                    if (err) {
                                        console.log(err);
                                    } else {
                            ...
                            case 'aboutMe':
                                console.log(msg.data);
                                con.query(`UPDATE main.userinfo SET aboutMe = '${msg.data}' WHERE username = '${msg.user}'`, function (err, result) {
                                    if (err) {
                                        console.log(err);
                                    } else {
                                        console.log('About Me Changed Successfully');
                                    }
                                });
                        }
                                break;
                    ...
                    case 'getData':
                        con.query(`SELECT * FROM main.userinfo WHERE username = '${msg.user}'`, function(err,result){
                            if(err){
                                console.log(err);
                            }else{
                                io.emit('html', JSON.stringify({type:'setData', pfp:result[0].profilePicture, location:result[0].location, aboutMe:result[0].aboutMe}));
                            }
                        });
                        break;
                    default:
